...
const channels = {
  receiver: new BroadcastChannel("obs-painelzinho-channel"),
  sender: new BroadcastChannel("obs-painelzinho-channel2"),
  fonts: new BroadcastChannel("obs-painelzinho-fonts"),
};
const bcp = channels.receiver; // recebe mensagens do painelzinho
const bcr = channels.sender; // envia mensagens para o painelzinho
const bcf = channels.fonts; // recebe fontes dinâmicas

// Configuração dos tempos e flags (garanta que estejam definidos)
const alt_1_oneshot = false;
const alt_2_oneshot = false;
const alt_3_oneshot = false;
const alt_4_oneshot = false;
const alt_1_active_time = 10;
const alt_2_active_time = 10;
const alt_3_active_time = 10;
const alt_4_active_time = 10;
const alt_1_inactive_time = 5;
const alt_2_inactive_time = 5;
const alt_3_inactive_time = 5;
const alt_4_inactive_time = 5;

let alt_1_turnoff = false;
let alt_2_turnoff = false;
let alt_3_turnoff = false;
let alt_4_turnoff = false;
...
const lowerThirds = {
  ...
  "painelzinho-4": {
    ...
    inactiveIsOn: false,
    turnoff: false,
    oneshot: alt_4_oneshot,
    activeTime: alt_4_active_time,
    inactiveTime: alt_4_inactive_time,
  },
};
...
function changeLogoVisibility(...args) {
  for (let i = 0; i < 4; i++) {
    ...
    if (logoSwitch === "true") {
      removeClass(logoId, "no-logo");
      loadLogo(logoImgId, logoImage);
    } else {
      addClass(logoId, "no-logo");
      loadLogo(logoImgId);
    }
  }
}

function loadLogo(id, logo) {
  if (logo) {
    setSrc(id, logo);
  } else {
    setSrc(id, "//:0");
...
bcp.onmessage = (ev) => {
  ...
  const updateText = (id, text) => setText(id, text);
  ...
  const processAlt = (i) => {
    ...
    let border_thickness_amount =
      (getValue(prefix + "border_thickness_amount") || 0) * 0.1;
    ...
    const logo_image =
      getValue(prefix + "logo_image") || `../logos/logo_${i}.png`;
    ...
    let animation_time = Number(
      getValue(prefix + "animation_time") || global_animation_time
    );
    let active_time = Number(getValue(prefix + "active_time") || activeTime);
    let inactive_time = Number(
      getValue(prefix + "inactive_time") || inactiveTime
    );

    const oneshot =
      getValue(prefix + "oneshot") ||
      (global_oneshot && !getValue(prefix + "inactive_time"));
    ...
    if (align !== align_old[i]) {
      ...
      if (el)
        ["left", "center", "right"].forEach((cls) =>
          el.classList.replace(cls, align)
        );
      align_old[i] = align;
    }
    ...
    if (style !== style_old[i]) {
      if (switch_val === "false" && el) el.classList.add("hide-anim");
      if (el)
        ["style-1", "style-2", "style-3"].forEach((cls) =>
          el.classList.replace(cls, `style-${style}`)
        );
      style_old[i] = style;
    }
  ...
  changeLogoVisibility(
    altSettings[1].logo_switch,
    altSettings[1].logo_image,
    altSettings[2].logo_switch,
    altSettings[2].logo_image,
    altSettings[3].logo_switch,
    altSettings[3].logo_image,
    altSettings[4].logo_switch,
    altSettings[4].logo_image
  );
  ...
  alts.forEach((i) => {
    ...
    root.style.setProperty(
      `--${prefix}-animation-time`,
      alt.animation_time + "s"
    );
    ...
    properties.forEach((prop) => {
      ...
      if (prop === "size") {
        ...
        prop.includes("border_thickness_amount")
      ) {
        cssValue = value + "rem";
      } else if (
  ...
  alts.forEach((i) => {
    ...
    const bgZero =
      alt.style_color_2 && alt.style_color_2.match(/,0\)$/) ? "0" : "1";
  ...
  // Agora seu tratamento extra:
  if (data.action === "updateText") {
    setText("algum-id-no-html", data.value);
  }
};

// =======================
// 6. Recepção de Dados dos Canais - FIM
// =======================
...
const updateInterval = setInterval(refreshData, updateIntervalMs);
