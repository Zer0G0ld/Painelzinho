...
// Recupera o número de fontes customizadas do localStorage
const customFontsCount = Number(localStorage.getItem("customFonts")) || 0;

// Renderiza as fontes customizadas já salvas na lista
for (let i = 0; i < customFontsCount; i++) {
  const fontNumber = i + 1;
  $("#custom-font-list ul").append(`
    <li>
      <div class="font-name-added slot-storable" id="font-name-added-${fontNumber}"></div>
      <div class="font-url-added slot-storable" id="font-url-added-${fontNumber}"></div>
      <div class="delete-font"><div class="icon icon-trash"></div></div>
    </li>
  `);
}

// Função para extrair o "shortname" da fonte (ex: 'Open Sans' de "'Open Sans', sans-serif")
function extractShortName(fontFullName) {
  // Tenta extrair a parte entre aspas simples
  const match = fontFullName.match(/'([^']+)'/);
  return match ? match[1] : fontFullName;
}

// Atualiza a lista de fontes customizadas e popula os selects
function updateCustomFontList() {
  // Atualiza as listas com as fontes customizadas do DOM
  ...
  // Para cada select com a classe 'font-storable', atualiza as opções
  $(".font-storable").each(function () {
    ...
    // Define o valor selecionado baseado no localStorage, ou valor padrão
    const savedValue = localStorage.getItem($select.attr("id"));
    if (savedValue && fontlistNames.includes(savedValue)) {
      $select.val(savedValue);
    } else {
      $select.val("'Open Sans', sans-serif");
    }
  });

  // Se quiser, aqui chama a função que envia as fontes para o painel
  // function_send_font();
}

// Função para adicionar uma nova fonte customizada e atualizar selects
function addNewFont(fontName, fontUrl) {
  ...
  const shortName = extractShortName(fontName);

  $(".font-storable").each(function () {
    $(this).append(`<option value="${fontName}">${shortName}</option>`);
  });

  // Atualiza localStorage ou qualquer outra coisa necessária
  // function_send_font();
}
// Remove fonte customizada dos arrays e dos selects
function removeFont(fontName, fontUrl) {
  ...
  // Remove das opções nos selects
  $(".font-storable").each(function () {
    $(this).find(`option[value="${fontName}"]`).remove();
  });
}

// Função para limpar strings de forma segura
...
// Adicionar nova fonte ao clicar no botão
$("#add-new-font").click(function () {
  let howMany = $("#custom-font-list ul li").length + 1;
  const fontNameRaw = $("#new-font-name").val();
  const fontUrlRaw = $("#new-font-url").val();

  if (fontNameRaw && fontUrlRaw) {
    $("#new-font-name").val("");
    $("#new-font-url").val("");

    const fontNameClean = cleanFontName(fontNameRaw);
    const fontUrlClean = cleanFontUrl(fontUrlRaw);
    ...
    let currentHeight = parseInt(globalConfig.style.maxHeight) || 0;
    currentHeight += 28;
    globalConfig.style.maxHeight = `${currentHeight}px`;
    moreSettings.style.maxHeight = `${currentHeight}px`;

    // Atualiza localStorage
    localStorage.setItem("customFonts", howMany);
    localStorage.setItem(`font-name-added-${howMany}`, fontNameClean);
    localStorage.setItem(`font-url-added-${howMany}`, fontUrlClean);
  }
});

// Remover fonte ao clicar no ícone de lixo
$("#custom-font-list ul").on("click", "li > .delete-font", function () {
  ...
  // Remove itens extras no localStorage acima do novo total
  for (let i = 20; i > howMany; i--) {
    localStorage.removeItem(`font-name-added-${i}`);
    localStorage.removeItem(`font-url-added-${i}`);
  }

  renameFontIds(); // Atualiza os IDs e localStorage após remoção
});

// Renomeia IDs dos fonts para manter sequência e salva no localStorage
function renameFontIds() {
  $("#custom-font-list ul li").each(function (index) {
    ...
    $(this).find(".font-name-added").attr("id", fontNameId);
    $(this).find(".font-url-added").attr("id", fontUrlId);

    localStorage.setItem(fontNameId, $(`#${fontNameId}`).text());
    localStorage.setItem(fontUrlId, $(`#${fontUrlId}`).text());
  });
}

// Atualiza os IDs quando a lista mudar (ex: remoção)
// Recomendo chamar renameFontIds() após qualquer alteração que modifique os itens da lista
...
if (localStorage.getItem("sorted") !== null) {
  var arrValuesForOrder = localStorage
    .getItem("sorted")
    .substring(6)
    .split("&alt[]=");
  var $ul = $("#sortable");
...
function checkSwitches() {
  ...
  if (document.getElementById("lower-thirds-masterswitch").checked) {
    ...
    document
      .getElementById("alt-main-config-content")
  ...
  if (masterSwitchIsOn) {
    ...
    if (document.getElementById("lower-thirds-switch1").checked) {
      if (
        alt_1_active_time_monitor >= 0 &&
        alt_1_inactive_time_monitor == 0 &&
        alt_1_name &&
        alt_1_info
      ) {
        document
          .getElementById("alt-1-config-content")
    ...
    if (document.getElementById("lower-thirds-switch2").checked) {
      if (
        alt_2_active_time_monitor >= 0 &&
        alt_2_inactive_time_monitor == 0 &&
        alt_2_name &&
        alt_2_info
      ) {
        document
          .getElementById("alt-2-config-content")
          .classList.remove("inactive");
    ...
    if (document.getElementById("lower-thirds-switch3").checked) {
      if (
        alt_3_active_time_monitor >= 0 &&
        alt_3_inactive_time_monitor == 0 &&
        alt_3_name &&
        alt_3_info
      ) {
        document
          .getElementById("alt-3-config-content")
          .classList.remove("inactive");
    ...
    if (document.getElementById("lower-thirds-switch4").checked) {
      if (
        alt_4_active_time_monitor >= 0 &&
        alt_4_inactive_time_monitor == 0 &&
        alt_4_name &&
        alt_4_info
      ) {
        document
          .getElementById("alt-4-config-content")
          .classList.remove("inactive");
...
// Função para alternar o estado do checkbox e disparar o evento change
...
// Array para os switches de toggle
const hotkeySwitches = [
  { hotkey: "hotkeyMasterSwitch", oldHotkey: "hotkeyMasterSwitchOld", id: "lower-thirds-masterswitch" },
  { hotkey: "hotkeySwitch1", oldHotkey: "hotkeySwitch1Old", id: "lower-thirds-switch1" },
  { hotkey: "hotkeySwitch2", oldHotkey: "hotkeySwitch2Old", id: "lower-thirds-switch2" },
  { hotkey: "hotkeySwitch3", oldHotkey: "hotkeySwitch3Old", id: "lower-thirds-switch3" },
  { hotkey: "hotkeySwitch4", oldHotkey: "hotkeySwitch4Old", id: "lower-thirds-switch4" },
];

// Dados dos memory slots organizados em arrays
const memorySlots = [
  {
    prefix: "#alt-1",
...
// Função genérica para processar esses memory slots
...
// Função que alterna checkbox e dispara .change()
function toggleCheckbox(id) {
  const $chk = $(`#${id}`);
  $chk.prop('checked', !$chk.prop('checked')).change();
}

// Função principal de checagem dos hotkeys
...
function checkLogos() {
  $(".alt-logo-preview").each(function () {
    ...
    check_is_default_logo =
      $("#" + check_logo_id).attr("src") ==
      $("#" + check_default_logo_id).attr("src")
        ? true
        : false;
...
function checkSlots(memory_slot, name, info, logo) {
  $("ul" + memory_slot + " li").each(function () {
    ...
    if (
      $("#" + check_name_id).is(":empty") &&
      $("#" + check_info_id).is(":empty") &&
      $("#" + check_logo_id).is(":empty")
    ) {
    ...
    if (
      $(name).val() == name_value &&
      $(info).val() == info_value &&
      ($(logo).attr("src") == logo_value ||
        (check_is_default_logo && logo_value == "default"))
    ) {
      if (name_value && info_value && logo_value) {
...
function memorySlotsSystem(memory_slot, name, info, logo) {
  ...
  $("ul" + memory_slot + " li")
    .mousedown(function (e) {
      ...
      is_default_logo =
        $(logo).attr("src") == $(default_logo).attr("src") ? true : false;
    ...
    .mouseup(function (e) {
      ...
      if (!slotDeleted) {
        if (
          ...
          //Load
          const name_to_load = $(this)
            .children("#" + name_id)
            .text();
          const info_to_load = $(this)
            .children("#" + info_id)
            .text();
          const logo_to_load = $(this)
            .children("#" + logo_id)
            .text();
          ...
          const auto_trigger = slot_id.replace(
            "-slot-" + slot_number_called,
            "-autotrigger"
          );
          ...
          if (
            ...
            $("#lower-thirds-switch" + alt_switch).is(":not(:checked)")
          ) {
            $("#lower-thirds-switch" + alt_switch)
              .prop("checked", true)
              .change();
          }
        }
...
function getAppearance() {
  ...
  //Logo Size
  alt_1_logo_size =
    Number($("#alt-1-logo-size").val()) * 0.25 + 3.5 || default_logo_size_em;
  alt_2_logo_size =
    Number($("#alt-2-logo-size").val()) * 0.25 + 3.5 || default_logo_size_em;
  alt_3_logo_size =
    Number($("#alt-3-logo-size").val()) * 0.25 + 3.5 || default_logo_size_em;
  alt_4_logo_size =
    Number($("#alt-4-logo-size").val()) * 0.25 + 3.5 || default_logo_size_em;

  //Shadow Amount
  alt_1_shadow_amount =
    $("#alt-1-shadow-amount").val() * 0.1 || default_shadow_amount_em;
  alt_2_shadow_amount =
    $("#alt-2-shadow-amount").val() * 0.1 || default_shadow_amount_em;
  alt_3_shadow_amount =
    $("#alt-3-shadow-amount").val() * 0.1 || default_shadow_amount_em;
  alt_4_shadow_amount =
    $("#alt-4-shadow-amount").val() * 0.1 || default_shadow_amount_em;
  ...
  alt_1_border_thickness_amount =
    $("#alt-1-border-thickness-amount").val() ||
    default_border_thickness_amount;
  alt_2_border_thickness_amount =
    $("#alt-2-border-thickness-amount").val() ||
    default_border_thickness_amount;
  alt_3_border_thickness_amount =
    $("#alt-3-border-thickness-amount").val() ||
    default_border_thickness_amount;
  alt_4_border_thickness_amount =
    $("#alt-4-border-thickness-amount").val() ||
    default_border_thickness_amount;
...
function styleRestrictions() {
  switch (alt_1_style) {
    case "1":
      ...
      $("#alt-1-style-border-color-appearance > :nth-child(3)").addClass(
        "disabled"
      );
    ...
    case "2":
      ...
      // document.getElementById("alt-1-background").classList.remove("locked");
      $("#alt-1-style-border-color-appearance > :nth-child(3)").removeClass(
        "disabled"
      );
  ...
  switch (alt_2_style) {
    case "1":
      ...
      $("#alt-2-style-border-color-appearance > :nth-child(3)").addClass(
        "disabled"
      );
      break;
    case "2":
      ...
      $("#alt-2-style-border-color-appearance > :nth-child(3)").removeClass(
        "disabled"
      );
      break;
    case "3":
  ...
  switch (alt_3_style) {
    case "1":
      ...
      $("#alt-3-style-border-color-appearance > :nth-child(3)").addClass(
        "disabled"
      );
      break;
    case "2":
      ...
      $("#alt-3-style-border-color-appearance > :nth-child(3)").removeClass(
        "disabled"
      );
      break;
    case "3":
  ...
  switch (alt_4_style) {
    case "1":
      ...
      $("#alt-4-style-border-color-appearance > :nth-child(3)").addClass(
        "disabled"
      );
      break;
    case "2":
      ...
      $("#alt-4-style-border-color-appearance > :nth-child(3)").removeClass(
        "disabled"
      );
      break;
    case "3":
...
bcr.onmessage = function (ev) {
  ...
  const bcr = new BroadcastChannel("obs-painelzinho-channel2"); // Recebe do browser-source.js
  const bcp = new BroadcastChannel("obs-painelzinho-channel"); // Envia para browser-source.js
  ...
  if (received_data.resend) {
    ...
    document.getElementById("alt-1-active-time-monitor").innerHTML =
      alt_1_active_time_monitor;
    document.getElementById("alt-1-inactive-time-monitor").innerHTML =
      alt_1_inactive_time_monitor;
    document.getElementById("alt-2-active-time-monitor").innerHTML =
      alt_2_active_time_monitor;
    document.getElementById("alt-2-inactive-time-monitor").innerHTML =
      alt_2_inactive_time_monitor;
    document.getElementById("alt-3-active-time-monitor").innerHTML =
      alt_3_active_time_monitor;
    document.getElementById("alt-3-inactive-time-monitor").innerHTML =
      alt_3_inactive_time_monitor;
    document.getElementById("alt-4-active-time-monitor").innerHTML =
      alt_4_active_time_monitor;
    document.getElementById("alt-4-inactive-time-monitor").innerHTML =
      alt_4_inactive_time_monitor;
...
function function_send() {
  ...
  global_lock_active = document.getElementById("global-lock-active").checked
    ? true
    : false;

  alt_1_lock_active = document.getElementById("alt-1-lock-active").checked
    ? true
    : false;
  alt_2_lock_active = document.getElementById("alt-2-lock-active").checked
    ? true
    : false;
  alt_3_lock_active = document.getElementById("alt-3-lock-active").checked
    ? true
    : false;
  alt_4_lock_active = document.getElementById("alt-4-lock-active").checked
    ? true
    : false;
  ...
  alt_1_autoload = document.getElementById("alt-1-autoload").checked
    ? true
    : false;
  alt_2_autoload = document.getElementById("alt-2-autoload").checked
    ? true
    : false;
  alt_3_autoload = document.getElementById("alt-3-autoload").checked
    ? true
    : false;
  alt_4_autoload = document.getElementById("alt-4-autoload").checked
    ? true
    : false;
  ...
  global_oneshot = document.getElementById("global-oneshot").checked
    ? true
    : false;

  alt_1_oneshot = document.getElementById("alt-1-oneshot").checked
    ? true
    : false;
  alt_2_oneshot = document.getElementById("alt-2-oneshot").checked
    ? true
    : false;
  alt_3_oneshot = document.getElementById("alt-3-oneshot").checked
    ? true
    : false;
  alt_4_oneshot = document.getElementById("alt-4-oneshot").checked
    ? true
    : false;
        ...
        if (isStored) {
          $(this).prepend(
            '<span class="tooltiptext">' +
              ...
              "<br>" +
              info_to_tooltip +
              "<hr><p>Click and hold to delete.</p></span>"
          );
        }
      }
    });
  ...
  if (document.getElementById("alt-1-logo").checked == false) {
    ...
    $(
      "#alt-1-config-content > .first-edit-container > .logo-container"
    ).addClass("hide");
  } else {
  ...
  if (document.getElementById("alt-2-logo").checked == false) {
    ...
    $(
      "#alt-2-config-content > .first-edit-container > .logo-container"
    ).addClass("hide");
  } else {
  ...
  if (document.getElementById("alt-3-logo").checked == false) {
    ...
    $(
      "#alt-3-config-content > .first-edit-container > .logo-container"
    ).addClass("hide");
  } else {
  ...
  if (document.getElementById("alt-4-logo").checked == false) {
    ...
    $(
      "#alt-4-config-content > .first-edit-container > .logo-container"
    ).addClass("hide");
  } else {
  ...
  if (
    ...
    (alt_1_active_time && alt_1_active_time != "Infinity") ||
    alt_1_inactive_time ||
    alt_1_lock_active ||
    alt_1_oneshot
  ) {
  ...
  if (
    ...
    (alt_2_active_time && alt_2_active_time != "Infinity") ||
    alt_2_inactive_time ||
    alt_2_lock_active ||
    alt_2_oneshot
  ) {
  ...
  if (
    ...
    (alt_3_active_time && alt_3_active_time != "Infinity") ||
    alt_3_inactive_time ||
    alt_3_lock_active ||
    alt_3_oneshot
  ) {
  ...
  if (
    ...
    (alt_4_active_time && alt_4_active_time != "Infinity") ||
    alt_4_inactive_time ||
    alt_4_lock_active ||
    alt_4_oneshot
  ) {
  ...
  bc.postMessage({
    ...
    alt_2_name_size,
    alt_2_info_size,
    alt_2_name_transform,
    alt_2_info_transform,
    alt_2_name_weight,
    alt_2_info_weight,
    alt_2_name_color,
    alt_2_info_color,
    ...
    alt_3_name_size,
    alt_3_info_size,
    alt_3_name_transform,
    alt_3_info_transform,
    alt_3_name_weight,
    alt_3_info_weight,
    alt_3_name_color,
    alt_3_info_color,
    ...
    alt_4_name_size,
    alt_4_info_size,
    alt_4_name_transform,
    alt_4_info_transform,
    alt_4_name_weight,
    alt_4_info_weight,
    alt_4_name_color,
    alt_4_info_color,
...
function settingsTooltips() {
  ...
  $(
    "#alt-1-animation-time, #alt-2-animation-time, #alt-3-animation-time, #alt-4-animation-time"
  ).attr("title", tooltip.animation_time);
  $(
    "#alt-1-active-time, #alt-2-active-time, #alt-3-active-time, #alt-4-active-time"
  ).attr("title", tooltip.active_time);
  $(
    "#alt-1-inactive-time, #alt-2-inactive-time, #alt-3-inactive-time, #alt-4-inactive-time"
  ).attr("title", tooltip.inactive_time);
  $(
    "#global-lock-active, #alt-1-lock-active, #alt-2-lock-active, #alt-3-lock-active, #alt-4-lock-active"
  )
    .parent()
    .attr("title", tooltip.lock_active);
  $(
    "#global-oneshot, #alt-1-oneshot, #alt-2-oneshot, #alt-3-oneshot, #alt-4-oneshot"
  )
    .parent()
    .attr("title", tooltip.oneshot);

  $("#alt-1-preview, #alt-2-preview, #alt-3-preview, #alt-4-preview")
    .parent()
    .attr("title", tooltip.preview);
  $(
    "#alt-1-align-left, #alt-2-align-left, #alt-3-align-left, #alt-4-align-left"
  )
    .next()
    .attr("title", tooltip.align_left);
  $(
    "#alt-1-align-center, #alt-2-align-center, #alt-3-align-center, #alt-4-align-center"
  )
    .next()
    .attr("title", tooltip.align_center);
  $(
    "#alt-1-align-right, #alt-2-align-right, #alt-3-align-right, #alt-4-align-right"
  )
    .next()
    .attr("title", tooltip.align_right);
  $("#alt-1-style, #alt-2-style, #alt-3-style, #alt-4-style")
    .parent()
    .parent()
    .attr("title", tooltip.style);
  $("#alt-1-size, #alt-2-size, #alt-3-size, #alt-4-size")
    .parent()
    .parent()
    .attr("title", tooltip.size);
  $("#alt-1-margin-h, #alt-2-margin-h, #alt-3-margin-h, #alt-4-margin-h")
    .parent()
    .parent()
    .attr("title", tooltip.margin);
  $(
    "#alt-1-inverse-ratio, #alt-2-inverse-ratio, #alt-3-inverse-ratio, #alt-4-inverse-ratio"
  )
    .parent()
    .parent()
    .attr("title", tooltip.text_size);
  $(
    "#alt-1-line-spacing, #alt-2-line-spacing, #alt-3-line-spacing, #alt-4-line-spacing"
  )
    .parent()
    .parent()
    .attr("title", tooltip.line_spacing);
  $("#alt-1-font, #alt-2-font, #alt-3-font, #alt-4-font")
    .parent()
    .attr("title", tooltip.font);
  $("#alt-1-logo, #alt-2-logo, #alt-3-logo, #alt-4-logo")
    .parent()
    .attr("title", tooltip.logo);
  $("#alt-1-logo-size, #alt-2-logo-size, #alt-3-logo-size, #alt-4-logo-size")
    .parent()
    .attr("title", tooltip.logo_size);
  $("#alt-1-shadows, #alt-2-shadows, #alt-3-shadows, #alt-4-shadows")
    .parent()
    .attr("title", tooltip.shadow);
  $(
    "#alt-1-shadow-amount, #alt-2-shadow-amount, #alt-3-shadow-amount, #alt-4-shadow-amount"
  )
    .parent()
    .attr("title", tooltip.shadow_opacity);
  $(
    "#alt-1-background, #alt-2-background, #alt-3-background, #alt-4-background"
  )
    .parent()
    .attr("title", tooltip.fill_colors);
  $("#alt-1-corners, #alt-2-corners, #alt-3-corners, #alt-4-corners")
    .parent()
    .parent()
    .attr("title", tooltip.corners);
  $(
    "#alt-1-border-thickness, #alt-2-border-thickness, #alt-3-border-thickness #alt-4-border-thickness"
  )
    .parent()
    .attr("title", tooltip.borders);
  $(
    "#alt-1-border-color, #alt-2-border-color, #alt-3-border-color, #alt-4-border-color"
  )
    .parent()
    .attr("title", tooltip.borders);
  $(
    "#alt-1-border-thickness-amount, #alt-2-border-thickness-amount, #alt-3-border-thickness-amount, #alt-4-border-thickness-amount"
  )
    .parent()
    .attr("title", tooltip.borders_thinkness);

  $(
    "#alt-1-clean-inputs, #alt-2-clean-inputs, #alt-3-clean-inputs, #alt-4-clean-inputs"
  ).attr("title", tooltip.clean);
  $(
    "#alt-1-autotrigger, #alt-2-autotrigger, #alt-3-autotrigger, #alt-4-autotrigger"
  )
    .parent()
    .attr("title", tooltip.autotrigger);
  $("#alt-1-autoload, #alt-2-autoload, #alt-3-autoload, #alt-4-autoload")
    .parent()
    .attr("title", tooltip.autoload);
  $(
    "#alt-1-custom-times, #alt-2-custom-times, #alt-3-custom-times, #alt-4-custom-times"
  ).attr("title", tooltip.custom_times);
